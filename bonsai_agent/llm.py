...
class BonsaiLLM:
    ...
    def bind(self,observer=None,role="unknown"):
        self.observer=observer; self.role=role; return self
    ...
    def chat(self,messages,max_tokens=3000,temperature=0.2):
        ...
        if self.observer:
            self.observer({'role':self.role,'usage':usage,'timings':timings,'seconds':elapsed,'max_tokens':max_tokens,'temperature':temperature})
