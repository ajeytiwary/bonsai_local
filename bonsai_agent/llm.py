...
class BonsaiLLM:
 ...
 def _post(self,payload,max_tokens,temperature,constrained=False,retries=2):
  last=None
  for attempt in range(retries+1):
   start=time.perf_counter()
   try:
    res=requests.post(self.url,json=payload,timeout=self.timeout); elapsed=time.perf_counter()-start
    if res.status_code>=500:
     last=requests.HTTPError(f"{res.status_code} server error: {res.text[:1000]}",response=res)
     if attempt<retries: time.sleep(1.5*(attempt+1)); continue
     raise last
    res.raise_for_status(); data=res.json(); break
 ...
 def tool_turn(self,messages,max_tokens=1800,temperature=0.2,reasoning_budget=2048,tools=None):
